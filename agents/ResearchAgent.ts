 ...
 *
 */
const ResearchAgent = async (topic: string): Promise<string> => {
  console.log({ topic });
  try {
    ...
    // We'll create an LLM chain which is just a prompt template and an LLM or chat model
    const llmChain = new LLMChain({
      prompt: chatPrompt,
      llm: chat,
    });
    // then we'll use that LLM chain as the basis of the agent
    // so basically our agent is made up of: Tools, LLM, and Prompt Templates, making it highly customizable to our needs!
